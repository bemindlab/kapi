...
suite('Notebook builtin output renderer', () => {
	...
	function createOutputItem(text: string, mime: string, id?: string, appendedText?: string): OutputWithAppend {
		const resolvedId = id ?? 'test-output';
		return {
			id: resolvedId,
			mime: mime,
			appendedText() {
	...
	async function renderOutput(renderer: RendererApi, outputItem: OutputWithAppend, element: HTMLElement) {
		const controller = new AbortController();
		await renderer.renderOutputItem(outputItem, element, controller.signal);
	}
	...
	textLikeMimeTypes.forEach((mimeType) => {
		test(`Render with wordwrap and scrolling for mimetype ${mimeType}`, async () => {
			...
			await renderOutput(renderer, outputItem, outputElement);
		...
		test(`Replace content in element for mimetype ${mimeType}`, async () => {
			...
			await renderOutput(renderer, outputItem2, outputElement);
	...
	test(`Appending multiple streaming outputs`, async () => {
		...
		await renderOutput(renderer, outputItem1, firstOutputElement);
		const secondOutputElement = outputHtml.appendOutputElement();
		await renderOutput(renderer, outputItem2, secondOutputElement);
		const thirdOutputElement = outputHtml.appendOutputElement();
		await renderOutput(renderer, outputItem3, thirdOutputElement);

		const appendedItem1 = createOutputItem('', stdoutMimeType, '1', ' appended1');
		await renderOutput(renderer, appendedItem1, firstOutputElement);
		const appendedItem3 = createOutputItem('', stdoutMimeType, '3', ' appended3');
		await renderOutput(renderer, appendedItem3, thirdOutputElement);
	...
	test(`Multiple adjacent streaming outputs should be consolidated one element`, async () => {
		...
		await renderOutput(renderer, outputItem1, outputElement);
		await renderOutput(renderer, outputItem2, outputHtml.appendOutputElement());
		await renderOutput(renderer, outputItem3, outputHtml.appendOutputElement());
	...
	test(`Consolidated streaming outputs should replace matching outputs correctly`, async () => {
		...
		await renderOutput(renderer, outputItem2, secondOutput);
		const newOutputItem1 = createOutputItem('replaced content', stdoutMimeType, '2');
		await renderOutput(renderer, newOutputItem1, secondOutput);
	...
	test(`Consolidated streaming outputs should append matching outputs correctly`, async () => {
		...
		await renderOutput(renderer, appendingOutput, secondOutput);
	...
	test(`Multiple adjacent streaming outputs, rerendering the first should erase the rest`, async () => {
		...
		await renderOutput(renderer, newOutputItem1, outputElement);
	...
	test(`Settings event change listeners should not grow if output is re-rendered`, async () => {
		...
		const outputElement = new OutputHtml().getFirstOuputElement();
		await renderOutput(renderer, createOutputItem('content', stdoutMimeType), outputElement);
